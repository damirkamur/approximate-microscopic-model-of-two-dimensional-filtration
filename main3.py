...
# region пористость
def m_x_y_(x: float, y: float, min_m: float = 0.1, max_m: float = 0.4) -> float:
    """
    Функция пористости
    ...
    x = x / N
    y = y / N
    a = -1 + (math.cos(0.5 * x * (math.cos(5 + y)) ** 2)) ** 2
    b = x * math.sqrt(math.sinh(0.1 + y))
    c = a + b
    d = 1.35839 * (max_m - min_m)
    return min_m + c * d
...
# Вывод результатов
start = time()
with open(f'results3/psi{N}x{M}.dat', 'w') as file:
    for i in range(M + 1):
        for j in range(N + 1):
...
start = time()
with open(f'results3/p{N}x{M}.dat', 'w') as file:
    for i in range(M + 1):
        for j in range(N + 1):
            file.write('%15.6f' % (p[i][j]) + '\t' * 2)
        file.write('\n')
print(f'Выгрузка p: {time() - start} секунд')
with open(f'results3/m{N}x{M}.dat', 'w') as file:
    for i in range(M):
        for j in range(N):
