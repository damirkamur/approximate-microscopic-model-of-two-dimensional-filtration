...
def m_x_y_(x: float, y: float) -> float:
    ...
    """
    return m_i if (x - N / 2) ** 2 + y ** 2 <= N ** 2 / 36 else m_e
...
def psi_analit(x, y):
    ...
    """
    kappa = N / 6
    r = math.sqrt((x - N / 2) ** 2 + y ** 2)
    r /= kappa
    tetta = math.atan2(y, x - N / 2)
    if (x - N / 2) ** 2 + y ** 2 <= N ** 2 / 36:
        return kappa * (2 * ksi) / (1 + ksi) * r * math.sin(tetta)
    else:
        return kappa * (r + (ksi - 1) / ((1 + ksi) * r)) * math.sin(tetta)
...
m_e = 0.2
m_i = 2 * (ksi ** (1 / 3) - 1) * ksi ** (1 / 3) * ((1 - m_e) ** (1 / 2) - 1) + ksi ** (2 / 3) * m_e
m = np.array([[m_x_y_(x[i][j], y[i][j]) for j in range(N)] for i in range(M)])
...
# gu_from_function('top', 'p', lambda x: psi_analit(x, M))
# gu_const_val('bottom', 'q', 0)
# gu_const_val('left', 'q', 0)
# gu_from_function('right', 'p', lambda y: psi_analit(N, y))

gu_const_val('left', 'q', 1)
gu_const_val('right', 'q', 1)
gu_const_val('bottom', 'q', 0)
...
# Вывод результатов
start = time()
with open(f'results2/psi{N}x{M}.dat', 'w') as file:
    for i in range(M + 1):
        for j in range(N + 1):
...
start = time()
with open(f'results2/p{N}x{M}.dat', 'w') as file:
    for i in range(M + 1):
        for j in range(N + 1):
            file.write('%15.6f' % (p[i][j]) + '\t' * 2)
        file.write('\n')
print(f'Выгрузка p: {time() - start} секунд')
with open(f'results2/m{N}x{M}.dat', 'w') as file:
    for i in range(M):
        for j in range(N):
...
if analit_export:
    start = time()
    analit_psi = np.zeros((M + 1, N + 1))
    for i in range(M + 1):
        for j in range(N + 1):
            analit_psi[i][j] = psi_analit(j, i)
    with open(f'results2/analit_psi{N}x{M}.dat', 'w') as file:
        for i in range(M + 1):
            for j in range(N + 1):
                file.write('%15.6f' % (analit_psi[i][j]) + '\t' * 2)
            file.write('\n')
    print(f'Выгрузка аналитического psi: {time() - start} секунд')
